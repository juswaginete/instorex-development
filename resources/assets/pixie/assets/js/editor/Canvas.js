    ...
    let canvas = {
      ...
      start: function (url) {
        ...
        if (url) {
          ...
          const size = $rootScope.getParam('blankCanvasSize')
      ...
      mergeObjects: function () {
        ...
        const data = this.fabric.toDataURL()
        this.fabric.clear()
       ...
       */
      openImage: function (url, callback) {
        canvas.zoom(1)
        fabric.util.loadImage(url, function (image) {
          ...
          const maxWidth = canvas.mainImage ? canvas.mainImage.getWidth() : canvas.fabric.getWidth()
          const maxHeight = canvas.mainImage ? canvas.mainImage.getHeight() : canvas.fabric.getHeight()
          ...
          if (object.width >= maxWidth || object.height >= maxHeight) {
            ...
            const scale = 1 / (Math.min(newHeight / object.getHeight(), newWidth / object.getWidth()))
      ...
      fitToScreen: function () {
        const maxWidth = canvas.viewport.offsetWidth - 40
        const maxHeight = canvas.viewport.offsetHeight - 120
        const outter = canvas.mainImage || canvas.fabric
        const scale = Math.min(maxHeight / outter.getHeight(), maxWidth / outter.getWidth())
