...
angular.module('ImageEditor', ['ngMaterial', 'ngAnimate', 'ui.sortable', 'image.crop', 'image.text', 'image.drawing', 'image.filters', 'image.shapes', 'image.directives', 'image.basics'])
  ...
  .controller('MainController', ['$rootScope', '$scope', '$timeout', '$$rAF', '$mdBottomSheet', 'canvas', 'history', 'simpleShapes', 'filters', function ($rootScope, $scope, $timeout, $$rAF, $mdBottomSheet, canvas, history, simpleShapes, filters) {
    ...
    $rootScope.keys = {
      google_fonts: window.instorexSettings.googleKey
    }
    ...
    canvas.fabric.on('object:selected', function(e) {
        if (e.target.name === 'text') {
            ...
            for (let i = 0; i < simpleShapes.available.length; i++) {
                const shape = simpleShapes.available[i]
    ...
    canvas.fabric.on('object:modified', function (e) {
      const activeObject = e.target
      const last = history.getLatest('all')
      console.log(last);
      if (last != null && last.elementId === activeObject.$$hashKey) {
        history.change('Changed ' + activeObject.type, 'more', false, activeObject.$$hashKey)
      } else {
        history.add('Changed ' + activeObject.type, 'more', false, activeObject.$$hashKey)
      }
    })
